...
A pesar que todas las proteínas en la célula se originan de la misma forma en el momento de la síntesis en el ribosoma, debido a los diferentes caminos que puede tomar en el contexto celular,
una proteína puede llegar a encontrarse en situaciones muy distintas en la celula y cumpliendo roles muy diversos.
Para definir una proteína en términos biológicos se debe describir, entonces, las diferentes estados que puede poblar en la célula los cuales, además, pueden variar con el tiempo y el contexto celular.
...
La arquitectura de las proteínas, entonces, estará dada por combinaciones de distintos módulos distinguibles por propiedades secuenciales, estructurales o actividades biológicas características, 
...
al ser solubles y con una estructura determinada y relativamente estable, fueron las primeras que pudieron ser cristalizadas.
La anotación de las estructuras que se resolvían permitió conocer mas sobre las propiedades estructurales las cuales muestran, en términos generales, que la secuencia adopta una gran cantidad de estructuras secundarias junto con regiones
que, al parecer, no tienen una estructura determinada.
Las estructuras secundarias se unen mediante interacciones hidrofóbicas entre las cadenas laterales apolares, por puentes de hidrógeno y a través de interacciones iónicas entre aminoácidos cargados, dando lugar
...
donde los aminoácidos más hidrofóbicos quedan alojados principalmente en el interior del plegamiento, formando un núcleo hidrofóbico, y la superficie globular tiene mayoría de residuos polares, 
lo que le da la solubilidad característica en el entorno celular. 
...
El modelo nace de la necesidad por explicar la especificidad encontrada en el funcionamiento de diversas enzimas similares por sus correspondientes sustratos. 
...
Por lo tanto, a pesar de que estas proteínas puedan ser asociadas con estructuras estáticas definidas, las más mínimas fluctuaciones térmicas de la temperatura ambiente les permite 
explorar distintas conformaciones como parte de su actividad biológica.
...
Es decir, dado que en dominios globulares la estructura esta definida(podemos obtener un promedio del ensemble) y esta asociada a la función/actividad que desarrolla la proteina, 
esta estructura puede ser usada para clasificación y para inferir la funcionalidad en nuevas proteinas.
...
Los análisis de estructuras existentes asociadas a proteínas globulares permitieron encontrar que casi todas las proteínas presentaban similitudes estructurales 
...
El resultado de esto es, por ejemplo, las base de datos
SCOP y CATH, las cuales se basan en la topología emergente de la combinación de estructuras secundarias propia de la proteína para armar clasificaciones jerárquicas de la arquitectura.
...
Dado que la estructura asociada a la función debe ser mantenida evolutivamente para continuar cumpliendo la función, 
tanto la estructura como el proceso de plegamiento que requiere ésta, imponen un conjunto de restricciones sobre la evolución de la secuencia. 
Por lo tanto, la similitud secuencial también permite inferir propiedades funcionales. 
Esto facilita la anotación de elementos funcionales en secuencias nuevas, aún cuando no se conoce su estructura.
...
% ACTIVIDADES
La funcionalidad de las proteínas globulares depende directamente de la estrucura tridimensional dinámica
– Flexibilidad
...
En general, las funciones más estudiadas están asociadas con actividades enzimáticas.
...
Con el tiempo se encontraron proteínas que, al parecer, no se adaptaban a las características de plegamiento 
...
% junto con la aparición de diversas proteínas funcionales sin una estructura plegada, llevaron a centrar la atención en 
% segmentos/proteínas completas que no se adaptan correctamente al modelo clásico de plegamiento.
...
Como se describió previamente, la secuencia primaria de una proteina globular codifica el proceso de plegamiento y la estructura única final. 
De forma similar, al identificar la IDPs cómo entidades estructuralmente diferenciables, se podría pensar que la estructura primaria de éstas determina, también, su falta de estructura tridimensional definida.
...
La pregunta mas importante, entonces, es la función fisiológica que cumplen las IDPs y como es su mecanismo funcional.
...
Este mecanismo/estrategia de reconocimiento es básicamente diferente a la que adoptan los dominios globulares.
...
en IDPs el \textit{binding} es principalmente mediado por elementos(motivos) cortos de reconocimiento\cite{neduva2005systematic,fuxreiter2007local}, caracterizados por su naturaleza flexible, lo que 
...
Estos segmentos, que denominamos elemento de reconocimiento(RE=recognition element), pueden ser identificados desde sus propiedades conformacionales, involucran regiones cortas(10-70 aminoácidos de largo) con características anfipáticas,
y son fundamentales para la actividad de una gran cantidad de IDPs\cite{mohan2006analysis,vacic2007characterization,oldfield2005coupled}.
...
Además, los dos modelos descritos antes para el proceso de unión y plegamiento (selección de conformación y unión y plegamiento simultáneo) permiten subclasificar aún más estos elementos desde el punto de vista estructural, agregando el concepto de 
elementos estructurales preformados(PSEs)\cite{fuxreiter2004preformed}. 
Estos PSEs se corresponden con el modelo de selección de conformaciones, es decir, son regiones intrínsecamente desordenadas que tienen tendencia a formar elementos estructurales transitivos, adquiriendo termporalmente estructuras secundarias.
...
% ACA CONECTO CON SLM
Más alla de esta posibiliad de identificar regiones funcionales desde el punto de vista estructural, el mecanismo de reconocimiento ha sido estudiado desde el punto de vista
de los patrones secuenciales que determinan las capacidades funcionales de éstos (interacciones o modificaciones enzimáticas). 
Estos elementos secuenciales se conocen como motivos lineales (o por sus siglas en inglés, LMs, ELMs, SLiMs o MiniMotifs)\cite{davey2012attributes}. 
Los motivos lineales se definen como módulos regulatorios encontrados en proteínas y que se caracterizan por presentar interfaces
...
Como resultado de este número limitado de contactos para interacciones con ligandos, los SLMs se unen con una afinidad relativa muy baja lo que resulta en una ventaja considerable 
...
Los primeros muestran una apariencia granular y consisten principalmente de proteínas en conformaciones desplegadas, aunque presentan ciertas regiones con formación de de hoja-$\beta$
permitiendo que las proteínas se mantenga unidas formando el agregado.
...
Si bien no nos vamos a enfocar en los detalles puntuales de la estructura, se sabe que estas diferencias que se ven en la conformación macromolecular son el reflejo de diferencias en las interacciones y arquitectura a nivel atómico.
Por otro lado, estas diferencias estructurales también se reflejan a nivel biológico: los distintos tipos de agregados tienen afinidades diferentes frente al conjunto de chaperonas y, además, 
...
% ESTRUCTURA
De los dos tipos de agregados presentados más interesante, probablemente, es el que resulta de la formación de fibrillas amiloides, ya que tienen propiedades que las hacen muy particulares.
...
% PROPIEDADES SECUENCIALES
A pesar de esta arquitectura claramente definida de las fibrillas amiloides, las proteínas que forman estos agregados son muy diversas, con muy poca similitud entre sus secuencias y sus conformaciones monoméricas en solución.
Lo que es más relevante, estudios con diversos péptidos y proteínas (no relacionados con aquellos encontrados como causantes de la amiloidosis sistemica) muestran que todo pueden formar, bajo las condiciones apropiadas, agregados con las 
...
esta habilidad para reordenar la conformación monomérica y agregarse en una arquitectura definida característica puede ser una propiedad intrínseca de las proteínas\cite{fandrich2002behaviour}.

El estado estructural amiloide parece ser, entonces, un estado generico de las proteinas, sin  embargo, la tendencia a formar este tipo de fibrillas amiloides si es un proceso que depende específicamente de la 
secuencia y algunos detalles estructurales a nivel atómico, así como la estabilidad relativa del estado amiloide, pueden ser tambien características fuertemenete dependientes de la secuencia.  
...
La pregunta clave es cómo influye la secuencia(o composición) de aminoácidos sobre esta tendencia.
...
Se sabe que las interacciones hidrofóbicas son escenciales para la compactación de la estructura lineal de un polipéptido.
